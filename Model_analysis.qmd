...
# Data loading: use user data if present, otherwise use synthetic data
if os.path.exists(user_data_dir_train) and os.path.exists(user_data_dir_pred):
    # Detect separators for user data
    train_sep = detect_separator(user_data_dir_train, dropout_column)
    pred_sep = detect_separator(user_data_dir_pred, dropout_column)
    
    train_df = pd.read_csv(user_data_dir_train, sep=train_sep, engine='python')
    pred_df = pd.read_csv(user_data_dir_pred, sep=pred_sep, engine='python')
    data_type = "instelling"
else:
    # Detect separators for synthetic data
    train_sep = detect_separator(synth_data_dir_train, dropout_column)
    pred_sep = detect_separator(synth_data_dir_pred, dropout_column)
    
    train_df = pd.read_csv(synth_data_dir_train, sep=train_sep, engine='python')
    pred_df = pd.read_csv(synth_data_dir_pred, sep=pred_sep, engine='python')
...
# Analyze missing data using helper function
missing_summary, total_missing, total_rows, total_cols, missing_cols = analyze_missing_data(train_processed)
...
print(f"- Percentage rijen met minstens één missing value: {(train_processed.isnull().any(axis=1).sum() / total_rows * 100):.1f}%\n")
...
sns.heatmap(train_processed.isnull(), yticklabels=False, cbar=False, cmap='viridis', ax=ax)
```
